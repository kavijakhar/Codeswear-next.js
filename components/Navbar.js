...
const Navbar = ({Logout, user, cart, addtocart, removefromcart, clearcart, subTotal }) => {
    ...
    return (
        <div className='flex flex-col md:flex-row md:justify-start justify-center md:py-8 py-3 shadow-md sticky top-0 z-10 bg-white'>
            ...
            <div className="cart flex md:top-9 top-4 items-center absolute mx-3  right-0 font-medium">
                <a onMouseOver={() => { setdropdown(true) }} onMouseLeave={() => setdropdown(false)}>
                    {dropdown && <div onMouseOver={() => { setdropdown(true) }} onMouseLeave={() => { setdropdown(false) }} className="absolute right-8 bg-pink-300 shadow-xl top-7 rounded-md px-5 w-36">
                        <ul>
                            <li className='py-1 hover:text-pink-500 text-white cursor-pointer'>My Account</li>
                            <li className='py-1 hover:text-pink-500 text-white cursor-pointer'>Orders</li>
                            <li onClick={Logout} className='py-1 hover:text-pink-500 text-white cursor-pointer'>Logout</li>
                        </ul>
                    </div>}
                    {user.value && <MdAccountCircle className='mx-2 text-2xl md:text-3xl cursor-pointer' />}
                </a>
                {!user.value && <Link href={'/login'}>
                    <button className='bg-pink-600 mx-2 text-white pl-1 pr-1 rounded-md space-x-1 py-1
                '>Login</button>
                </Link>}
